...
export default function FahrplanPage() {
  ...
  const toggleTopic = (sectionId: number, topicId: number) => {
    setSections((previous) => previous.map((section) => section.id === sectionId ? { ...section, topics: section.topics.map((topic) => topic.id === topicId ? { ...topic, completed: !topic.completed } : topic) } : section));
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        ...
        <section className="space-y-6">{sections.map((section) => { const sectionCompleted = section.topics.filter((topic) => topic.completed).length; const sectionTotal = section.topics.length; const sectionProgress = Math.round((sectionCompleted / sectionTotal) * 100); return <article key={section.id} className="overflow-hidden rounded-2xl bg-white shadow-lg transition-shadow duration-300 hover:shadow-xl"><div className={`bg-gradient-to-r ${section.color} p-6 text-white`}><div className="flex items-center gap-4"><span className="text-4xl" aria-hidden="true">{section.icon}</span><div className="flex-1"><h3 className="text-2xl font-bold">{section.title}</h3><p className="mt-1 text-white/90">{section.description}</p></div><div className="text-right"><div className="text-2xl font-bold">{sectionProgress}%</div><div className="text-sm text-white/80">{sectionCompleted}/{sectionTotal} Themen</div></div></div></div><div className="p-6"><div className="space-y-3">{section.topics.map((topic) => <div key={topic.id} className="flex items-start gap-3 rounded-lg p-3 transition-colors hover:bg-gray-50"><input type="checkbox" checked={topic.completed} onChange={() => toggleTopic(section.id, topic.id)} className="mt-1 h-5 w-5 shrink-0 rounded text-indigo-600 focus:ring-2 focus:ring-indigo-500" aria-label={`${topic.label} als abgeschlossen markieren`} />{topic.href ? <button onClick={() => router.push(topic.href!)} className={`flex-1 text-left font-medium underline-offset-4 hover:text-indigo-700 hover:underline ${topic.completed ? 'text-gray-400 line-through' : 'text-gray-700'}`}>{topic.label} <span className="ml-1 text-sm no-underline">→ Lernen</span></button> : <span className={`flex-1 ${topic.completed ? 'text-gray-400 line-through' : 'text-gray-700'}`}>{topic.label}</span>}</div>)}</div></div></article>; })}</section>
        <aside className="mt-8 text-center text-sm text-gray-500"><p>📚 Dein Lernweg bündelt die für dich aufbereiteten Inhalte zu den Studienabschnitten.</p><p className="mt-2">🎯 Arbeite die Kapitel in deinem Tempo durch und nutze die Übungen zur Selbstkontrolle.</p></aside>
      </main>
    </div>
  );
}
